...
```
        ...
        │  money-path alive · near-duplicates · fabricated       │
        │  claims · security                                     │
        └───────────────┬─────────────────────────────────────┘
...
|------|--------------|--------------|
| 🟢 **auto-heal** | stale search index, stale/missing race pages | Repair loop applies the safe fix → Layer 1 must stay green → commits. Logged. |
| 🟡 **needs you** | **all ratings/scores/tiers**, missing content, broken citations, bad discipline, malformed JSON, missing dual units, missing climb_profile, near-duplicate races, fabricated/unsupported claims | A fix is *proposed* into a branch/PR. You get a 1-click link in the digest. |
...
```bash
python3 scripts/immune_check.py            # fast, offline: schema/scoring + index + duplicates + money-path wiring + claims
python3 scripts/immune_check.py --regen     # regenerate index/pages first, then also check page parity
python3 scripts/immune_check.py --live       # also hit production (roadielabs.com) for dead money-path / 404s
...
It reuses your existing, battle-tested `scripts/validate_profile.py` (schema/
scoring source of truth), `scripts/audit_fabricated_claims.py` (anti-shill
claim checker), and `scripts/check_links.py` (live link checker) — it does
not reinvent them. It writes `immune/report.json` (latest full result) and
appends a run record to `immune/ledger.jsonl`. **It never edits, commits, or
deploys.**

### Note on `preflight.py`
`scripts/preflight.py` in this repo is a step-RUNNER — it subprocess-calls
pytest, `audit_colors.py`, `validate_citations.py`, `audit_race_data.py`,
`validate_blog_content.py`, `youtube_validate.py`, then the generators, then
(with `--deploy`) `push_wordpress.py` and post-deploy validation — stopping at
the first failure. It has no importable checks-with-an-errors-list interface
the way XC Ski Labs' `preflight.py` does. `immune_check.py` instead imports
`validate_profile.py` directly (it already returns a clean `list[str]` of
errors per profile) as its schema/scoring criteria source.
...
(Seeded already with three real resolved issues from the Jul 2026 whoops audit —
see `docs/whoops-audit-jul2026.md`: the sitewide dead-nav-links regression, the
stale-`/questionnaire/`-money-path 404, and the gravel→road rating-key remap that
zeroed 8/14 radar dimensions on every page.)
...
1. **Schedule the nightly agent.** A `/schedule` cloud routine running the Layer-2
   routine above. (Ask Claude: "create the immune nightly routine for Roadie Labs.")
...
3. **Deploy authority (default OFF — keep it off until you trust it).** With it off,
   ...
   `push_wordpress.py --deploy-content` for 🟢 fixes only — never for money-path changes.

## CI (the cheap always-on layer)
`.github/workflows/immune.yml` runs `immune_check.py --regen --fail-on red` on every
push + nightly — so a change that kills the money path or leaks a secret turns the
build red immediately, for free, without waiting for the nightly agent. It complements
the existing `.github/workflows/link-check.yml` (weekly live-site sweep) and `test.yml`
(pytest + citation validators).

---

## Cloned from
The shape is identical to XC Ski Labs' immune system; only the checks differ to
match this repo's real structure — `validate_profile.py` instead of an importable
`preflight.PreflightResult`, `fondo_rating` instead of `nordic_lab_rating`,
`wordpress/generate_neo_brutalist.py` instead of `generate_race_pages.py`,
`roadielabs.com` instead of `xcskilabs.com`.
