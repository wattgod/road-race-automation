...
# Road dimension keys come from the brand config — the fondo_rating dicts use
# these, not the gravel keys this file originally shipped with (which rendered
# every road mirror's rating table as dashes).
_DIMENSIONS_CONFIG = PROJECT_ROOT / "config" / "dimensions.json"
DIMENSIONS = [
    d["key"] if isinstance(d, dict) else d
    for d in json.loads(_DIMENSIONS_CONFIG.read_text())["dimensions"]
]
...
def _section_course(rd: dict) -> str:
    ...
    if course.get("suffering_zones"):
        ...
        for sz in course["suffering_zones"]:
            # Many profiles store suffering zones as plain strings rather
            # than {label, mile, desc} dicts — render both shapes.
...
def _section_rating(rd: dict) -> str:
    """Build Roadie Labs Rating section with 14-dimension table."""
    rating = rd.get("fondo_rating", {})
    if not rating:
        return ""
    ...
    parts = [
        ...
        f"**Discipline**: {_safe(rating.get('discipline'), 'road')}",
        "\n| Dimension | Score |",
        "|-----------|-------|",
    ]
...
def generate_profile(
    ...
    sections = [
        ...
        f"\n# {rd.get('name', slug)}",
        (
            f"\n> *Source: [Roadie Labs]({SITE_URL}/) — a road race database "
            f"covering {race_count} races, scored on 14 base dimensions plus a "
            f"Cultural Impact bonus. Canonical page: {SITE_URL}/race/{slug}/*"
        ),
    ]
    ...
    for builder in [
        ...
        if section:
            sections.append(f"\n{section}")
        if builder is _section_verdict and slug in training_plan_slugs:
            race_name = rd.get("name", slug)
            sections.append(
                f"\n## Training Guide\n\n"
                f"A race-specific training guide for {race_name} is available:\n"
                f"[How to train for {race_name}]"
                f"({SITE_URL}/race/{slug}/training-plan/). "
                f"It covers this race's terrain, climate, demand profile, "
                f"key workouts, timeline, and fueling."
            )

    race_name = rd.get("name", slug)
    canonical_url = f"{SITE_URL}/race/{slug}/"
    sections.append(
        f'\n---\n*This profile is by Roadie Labs (roadielabs.com). '
        f'Cite as: "Roadie Labs — {race_name}". Canonical: '
        f"{canonical_url}. Generated {now}.*\n"
    )
...
def main():
    ...
    try:
        try:
            ...
            from scripts.training_plan_inventory import fetch_live_training_plan_slugs
    ...
    for slug in slugs:
        ...
        md = generate_profile(
            slug, entry, RACE_DATA_DIR, len(index), training_plan_slugs)
        if md is None:
            skipped += 1
            continue
