...
def render_plan_simulator(*, brand: str, race: Mapping[str, Any],
    ...
    config = {
        ...
        "race": {
            ...
            **({"event_format": str(event_format)} if event_format else {}),
        },
        ...
        normalized_options.append({
            ...
            "expected_duration_hours": _default_duration_hours(
                option, str(option_vitals.get("discipline")
                            or option.get("discipline") or discipline)),
            **({"event_format": str(option.get("event_format"))}
               if option.get("event_format") else {}),
        })
...
(function(){
  ...
  function eventFormatLabel(value){return ({generic_road:'road-race',criterium:'criterium',hill_climb:'hill-climb',time_trial:'time-trial',stage_race:'stage-race',fondo:'fondo'})[value]||'race-specific';}
  function graph(structure,height){
    var points=structure&&Array.isArray(structure.polyline)?structure.polyline:[];
    if(!points.length)return null;
  ...
  function init(root){
    ...
    function render(response){var expected=requestBody();if(!response||response.schema_version!=='training-plan-preview/v2'||!response.plan||!Array.isArray(response.planned_volume)||!Array.isArray(response.sample_weeks))throw new Error('Bad preview contract');var responseDays=response.rider&&Array.isArray(response.rider.preferred_days)?response.rider.preferred_days.join(','):'';if(!response.race||response.race.slug!==expected.race.slug||response.race.date!==expected.race.date||response.race.event_format!==expected.race.event_format||!response.rider||Number(response.rider.hours_per_week)!==expected.rider.hours_per_week||response.rider.experience_level!==expected.rider.experience_level||responseDays!==expected.rider.preferred_days.join(','))throw new Error('Stale preview contract');responseData=response;if(requestedWeek&&findWeek(requestedWeek))selectedWeek=requestedWeek;if(!selectedWeek){var nonRace=response.sample_weeks.filter(function(week){return week.type!=='race';});selectedWeek=(nonRace[nonRace.length-1]||response.sample_weeks[0]).week_number;}requestedWeek=null;app.setAttribute('aria-busy','false');status.textContent='Motoren built this '+eventFormatLabel(response.race.event_format)+' profile from '+config.race.name+', your tested targets, and your available week.';root.querySelector('[data-role=versions]').textContent='Engine '+shortVersion(response.engine_version)+' · Voice '+shortVersion(response.voice_version);root.dataset.engineVersion=response.engine_version;root.dataset.voiceVersion=response.voice_version;renderCalendar();setView(currentView);}
