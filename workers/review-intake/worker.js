 ...
 * Validates, sends Resend notification to coaching team.
 */
...
// matti@roadielabs.com -- the address Mission Control's road sequences
// already send from today (roadielabs.com verified in Resend Jul 2026),
// so it's confirmed to deliver, unlike an unused address on the domain.
async function sendNotificationEmail(env, review) {
  ...
  try {
    const resp = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Roadie Labs Reviews <matti@roadielabs.com>',
    ...
    console.log('Review notification (resend):', resp.status, detail.slice(0, 200));
  } catch (error) {
